...
#wire1s = 'R8,U5,L5,D3'
#wire2s = 'U7,R6,D4,L4'
def find_intersection(wire1s, wire2s):
    wire1 = wire1s.split(',')
    wire2 = wire2s.split(',')
    pos1 = [0,0]
    pos2 = [0,0]
    ...
    for i in range(len(wire1)):
        ...
        if dir1 == 'U':
            pos1 = [pos1[0],pos1[1]+move1]
        elif dir1 == 'D':
            pos1 = [pos1[0], pos1[1] - move1]
        elif dir1 == 'L':
            pos1 = [pos1[0]-move1, pos1[1]]
        elif dir1 == 'R':
            pos1 = [pos1[0]+move1, pos1[1]]
        for p in range(len(wire2)):
            dir2 = wire2[p][0]
            move2 = int(wire2[p][1:])
            if dir2 == 'U':
                pos2 = [pos2[0], pos2[1] + move2]
            elif dir2 == 'D':
                pos2 = [pos2[0], pos2[1] - move2]
            elif dir2 == 'L':
                pos2 = [pos2[0] - move2, pos2[1]]
            elif dir2 == 'R':
                pos2 = [pos2[0] + move2, pos2[1]]
            if (dir1 == 'U' or dir1 == "D") and (dir2 == "U" or dir2 == "D"):
                pass
            elif (dir1 == 'L' or dir1 == "R") and (dir2 == "L" or dir2 == "R"):
                pass
            elif dir1 == "L":
                if temp1[0] >= temp2[0] >= pos1[0]:
                    if dir2 == "U":
                        if temp2[1] <= temp1[1] <= pos2[1]:
                            matches.append([pos2[0], pos1[1]])
                            jumps.append(dist1 + abs(temp1[0]-temp2[0]))
                    else:
                        if temp2[1] >= temp1[1] >= pos2[1]:
                            matches.append([pos2[0], pos1[1]])
                            jumps.append(dist1 + abs(temp1[0]-temp2[0]))
                ...
                if temp1[0] <= temp2[0] <= pos1[0]:
                    if dir2 == "U":
                        if temp2[1] <= temp1[1] <= pos2[1]:
                            matches.append([pos2[0], pos1[1]])
                            jumps.append(dist1 + abs(temp2[0]-temp1[0]))
                    else:
                        if temp2[1] >= temp1[1] >= pos2[1]:
                            matches.append([pos2[0], pos1[1]])
                            jumps.append(dist1 + abs(temp2[0]-temp1[0]))
            elif dir1 == "U":
                if temp1[1] <= temp2[1] <= pos1[1]:
                    if dir2 == "L":
                        if temp2[0] >= temp1[0] >= pos2[0]:
                            matches.append([pos1[0], pos2[1]])
                            jumps.append(dist1+ abs(temp2[1]-temp1[1]))
                    else:
                        if temp2[0] <= temp1[0] <= pos2[0]:
                            matches.append([pos1[0], pos2[1]])
                            jumps.append(dist1 + abs(temp2[1]-temp1[1]))
                ...
                if temp1[1] >= temp2[1] >= pos1[1]:
                    if dir2 == "L":
                        if temp2[0] >= temp1[0] >= pos2[0]:
                            matches.append([pos1[0], pos2[1]])
                            jumps.append(dist1 + abs(temp1[1] - temp2[1]))
                    else:
                        if temp2[0] <= temp1[0] <= pos2[0]:
                            matches.append([pos1[0], pos2[1]])
                            jumps.append(dist1 - abs(temp1[1] - temp2[1]))
    ...
    res = [abs(x[0])+abs(x[1])for x in matches]
    temp = res.copy()
    res.sort()
    num = temp.index(res[0])
    return jumps[num], res[0]
...
if __name__ == "__main__":
    j, pos1 = find_intersection(w1, w2)
    d, pos2 = find_intersection(w2, w1)
    print(d+j)
    print(j, d)
    print(pos1, pos2)
